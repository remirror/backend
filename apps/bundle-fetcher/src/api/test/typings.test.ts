...
describe('fetchTypings', () => {
  ...
  it('includes src directory files for @angular/core', async () => {
    const result = await typings.downloadDependencyTypings('@angular/core@7.0.0');

    expect(result['/@angular/core/src/application_tokens.d.ts']).not.toBeFalsy();
  });

  it('includes .d.ts files for @dojo/framework', async () => {
    const result = await typings.downloadDependencyTypings('@dojo/framework@5.0.0');

    expect(result['/@dojo/framework/stores/Store.d.ts']).not.toBeFalsy();
  }, 10000);

  it("doesn't return a 404 for reallystate", async () => {
    const result = await typings.downloadDependencyTypings('reallystate@1.0.11');
  ...
  it('can download single package.json for no types', async () => {
    const result = await typings.downloadDependencyTypings('@styled-system/css@5.1.5');
  ...
  it('should clean old dependencies', async () => {
    ...
    typings.packageInstalls['old-dependency'] = Date.now() - typings.cleanUpTime - 1;
